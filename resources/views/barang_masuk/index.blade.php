...
<div class="max-w-[720px] mx-auto py-32 sm:py-8 lg:py-16">
  ...
  <div class="w-full flex justify-between items-center mb-3 mt-1 pl-3">
      <div>
          <h3 class="text-lg font-semibold text-slate-800">Data barang_masuks</h3>
          <p class="text-slate-500">A list of all the barang_masuks acount..</p>
      </div>
      <div class="ml-3">
          ...
          <div class="relative">
              ...
              placeholder="Search for barang_masuk data..."
              />
              <button
  ...
  <table class="w-full text-left table-auto min-w-max">
      ...
      <tr class="hover:bg-slate-50 border-b border-slate-200">
          ...
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $barang_masuk->nama_barang }}</p>
          </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $barang_masuk->tgl_masuk}}</p>
          </td>
          <td class="p-4 py-5">
            <p class="text-sm text-slate-500">{{ $barang_masuk->jml_masuk}}</p>
            </td>
            <td class="p-4 py-5">
                <p class="text-sm text-slate-500">{{ $barang_masuk->id_suplier }}</p>
                </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500"><a href="{{ route('barang_masuk.edit', $barang_masuk->id) }}" class="font-medium text-blue-600 hover:text-blue-800">Edit</a></p>
          </td>
          <td class="p-4 py-5">
           <p class="text-sm text-slate-500">
               <a href="{{ route('barang_masuk.destroy', $barang_masuk->id) }}"
                ...
                <form id="delete-form-{{ $barang_masuk->id }}" action="{{ route('barang_masuk.destroy', $barang_masuk->id) }}" method="POST" style="display: none;">
                   @csrf
                   @method('DELETE')
               </form>
           </p>
